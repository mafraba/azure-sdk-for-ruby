  ...
  module SqlDatabaseManagement
    class SqlDatabaseManagementService < BaseManagementService
      ...
      def list_servers
        ...
        request = SqlManagementHttpRequest.new(@config, :get, request_path, nil)
      ...
      def create_server(login, password, location)
        ...
        request_path = '/servers'
        request = SqlManagementHttpRequest.new(@config, :post, request_path, body)
        response = request.call
      ...
      def delete_server(name)
        if get_sql_server(name)
          ...
          request = SqlManagementHttpRequest.new(@config, :delete, request_path)
          request.call
      ...
      def list_sql_server_firewall_rules(server_name)
        if get_sql_server(server_name)
          ...
          request = SqlManagementHttpRequest.new(@config, :get, request_path)
          response = request.call
          Serialization.database_firewall_from_xml(response, @config.sql_database_authentication_mode)
        end
      end
